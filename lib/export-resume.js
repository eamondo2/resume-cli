...
const createPdf = (resumeJson, fileName, theme, format, callback) => {
  (async () => {
    ...
    let totalPage = await page.$('html');
    let boundingBox = await totalPage.boundingBox();

    console.log(boundingBox.height);
    console.log(boundingBox.width);

    await page.setViewport({
      width: Math.round(boundingBox.width * 2),
      height: Math.round(boundingBox.height),
    });

    totalPage = await page.$('html');
    boundingBox = await totalPage.boundingBox();

    console.log(boundingBox.height);
    console.log(boundingBox.width);

    const desiredTopMargin = 5;
    const desiredBottomMargin = 5;
    ...
    await page.pdf({
      ...
      height: `${
        boundingBox.height * 1.05 + desiredBottomMargin + desiredTopMargin
      }px`,
      pageRanges: '1',
      margin: {
        top: `${desiredTopMargin}px`,
        bottom: `${desiredBottomMargin}px`,
      },
